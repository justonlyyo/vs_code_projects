...
init_reg = function(seconds, style_class){
    ...
    var style_class = style_class === undefined? 'show_count': style_class;  // 倒计时时候的样式
    // 绑定短信发送事件
    ...
    sms_btns.each(function(){
        ...
        sms_btn.click(function(){
            ...
            if(validate_phone(user_phone)){
                var args = {"user_phone": user_phone};
                $.post("http://39.108.67.178:5000/send_sms", args, function(json){
    ...
    $(".reg_div").each(function () {
        ...
        var customer_description_item = div.find(".customer_description").first();
        var submit_btn = div.find(".submit_btn");
        
        /*检查元素是否存在?不存在的元素就不提交了*/
        if (user_phone_item.length === 0 || submit_btn.length === 0) {
            ...
            submit_btn.click(function () {
                ...
                var customer_description = $.trim(customer_description_item.val()) === '' ? "页面标题:" + $("title").text() : $.trim(customer_description_item.val()) === '';
                var mes = '';
                ...
                if (mes === '') {
                    ...
                    console.log(args);
                    $.post("http://39.108.67.178:5000/register", args, function (json) {
